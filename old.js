...
var employeeSalaryTable = function () {
	...
	var addEmployee = function ( newEmployee ) {
		...
		if( data.push( newEmployee ) ) return true;
	...
	this.add = function( obj, callback ) {
		...
		if( addEmployee( obj ) && typeof callback === "function" ) {
			return callback();
		}
	};
...
var salaryTable = new employeeSalaryTable();


salaryTable.submitForm = function() {
		...
		this.add( newEmployee, function() {
			...
			salaryTable.refresh();
			salaryTable.totalMonthlySalary();
		} );
		
};

salaryTable.totalMonthlySalary = function () {

	var employeeData = this.getData();
	...
	for ( var i = 0; i < employeeData.length; i++ ) {
		...
		monthlyExp += Math.round( employeeData[i].salary / 12 );

	}

	// Update Total of Monthly Outgoing Salalry
